...
<template>
  <!-- Eyebrow data -->
  <EyebrowComponent v-if="state !== 'done'" :last-updated="props.lastUpdated" :category="props.category" />
  <span v-else class="eyebrow">Thank you for voting!</span>
  <div class="card__thumbs-container">
    <button v-if="state !== 'done'" class="icon-button card-thumb-button" :class="{ 'selected': vote === 'positive' }" aria-label="thumbs up" @click="onHandleState('positive')">
      <img src="/assets/img/thumbs-up.svg" alt="thumbs up" />
    </button>
    <button v-if="state !== 'done'" class="icon-button card-thumb-button" :class="{ 'selected': vote === 'negative' }" aria-label="thumbs down" @click="onHandleState('negative')">
      <img src="/assets/img/thumbs-down.svg" alt="thumbs down" />
    </button>
    <button type="submit" :disabled="state === 'idle'"
      :title="state === 'idle' ? 'You must thumb up/down first' : 'Click to vote'" @click="onHandleSubmit" v-text="dynamicSubmitText" />
  </div>
</template>
